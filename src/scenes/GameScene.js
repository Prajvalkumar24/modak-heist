...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Arena docked directly below the 56px header
    this.arena = {
      x: 20,
      y: 56,
      w: width - 40,
    ...
    // 4. Temple Walls & Pillars (Strictly non-overlapping)
    this.walls = this.physics.add.staticGroup();
    this.spawnStructuredMap();

    // 5. Lord Ganesha Altar with Clean Altar-Side Lamps
    ...
    // Two lamps mounted directly beside Ganesha's shrine
    ...
    // 8. Modaks (Distributed evenly, no clumping, no wall overlap)
  ...
  createHeaderHUD(width) {
    ...
    this.levelBadge = this.add.text(40, 26, `LEVEL ${this.currentLevel}`, {
      fontSize: '18px',
      fontStyle: 'bold',
      color: '#ffb703',
      fontFamily: 'Verdana'
    ...
    this.scoreText = this.add.text(width / 2, 26, `Modaks: 0 / ${this.targetModaks}`, {
      fontSize: '18px',
      fontStyle: 'bold',
      color: '#ffffff',
      fontFamily: 'Verdana'
    ...
    this.timerText = this.add.text(width - 40, 26, `Aarti: ${this.timeLeft}s`, {
      fontSize: '18px',
      fontStyle: 'bold',
      color: '#06d6a0',
      fontFamily: 'Verdana'
  ...
  spawnStructuredMap() {
    ...
    // Sanctum Enclosure with 60px Open Doorway
    const cx = a.x + a.w / 2;
    const cy = a.y + 64;
    const sanctumWalls = [
    ...
    // Sector-Based Pillars (Strictly separated zones to guarantee zero overlap)
    const sectors = [
  ...
  spawnSafeModaks() {
    ...
    while (placed < this.targetModaks && attempts < 400) {
      ...
      // Distance check: at least 70px from other modaks
      const tooClose = placedPoints.some(pt => Phaser.Math.Distance.Between(mx, my, pt.x, pt.y) < 70);
      if (tooClose) continue;

      // Clearance check: 24px safety box around walls
